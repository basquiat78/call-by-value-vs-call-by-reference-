...
이것은 자바의 힙/스택 영역에 대해서 이해를 해야 한다.

예를 들면 메서드가 호출될 때는 스택에 올라가게 된다. 이때 지역변수나 매개변수, 즉 파라미터 등이 이 스택에 올라가게 된다.      

만일 매개변수 타입이 `참조 타입`이라면 `Test test = new Test();`같은 코드로 인해 힙메모리에 올라갈 것이고 이 힙 영역에 있는 객체의 주소값을 참조하게 된다.

이런 매카니즘으로 인해 넘길 때의 `test`와 메서드내의 `t`는 전혀 다른 녀석이 된다. 다만 레퍼런스의 사본이 넘어갔으니 객체가 가르키는 레퍼런스 주소는 같을 것이다.

따라서 `test.setName("before");`코드의 결과는 동일한 레퍼런스 주소값을 보고 있기 때문에 값이 변경된다. 

하지만 아까 말한대로 `t`는 복사된 객체이다. 

메서드가 실행될 때 스택에 올라 갈것이고 복사된 `t`는 메서드내에서만 유효한 변수일 테니 힙이 아닌 스택에 올라갈 것이다. 

따라서 이 변수 `t`는 해당 메서드내에서만 유효하고 메서드가 종료되면 스택에서 빠진다.     

그래서 이 `t`에 빈 객체를 할당한다해도 그것은 메서드가 끝나는 순간 유효하지 않는 것이다.
